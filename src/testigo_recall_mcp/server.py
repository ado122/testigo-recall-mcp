...
_STATIC_INSTRUCTIONS = (
    ...
    "- User asks about impact of changing a shared library or repo ('what depends on go-packages?') "
    "→ use direction='incoming'\n"
    "- User asks what a repo depends on or why a build broke across repos "
    "→ use direction='outgoing'\n"
    "- User asks about architecture, repo relationships, or system overview "
    "→ call with no repo_name to get the full graph\n"
    "- User modifies a package manifest (go.mod, package.json) and wants to know the blast radius\n"
    "- User asks 'what repos are connected?' or 'show me the dependency graph'\n"
    "Data comes from go.mod and package.json parsing — not AI analysis. "
    "Results show from_repo, to_repo, manifest source, and raw import string.\n\n"
